...
def sync_events(
    ...
    try:
        ...
        token_record = (
            ...
            .filter(GoogleCalendarToken.user_id == user_uuid)
            .first()
        )
        ...
        creds = Credentials(  # type: ignore[no-untyped-call]
            ...
            client_secret=os.environ.get("GOOGLE_CLIENT_SECRET", "mock_secret"),
            expiry=(
                token_record.expiry.replace(tzinfo=None)
        ...
        for app_event in real_events:
            ...
            mapping = (
                db.query(GoogleEventSync)
                .filter_by(user_id=user_uuid, app_event_id=app_evt_id_str)
                .first()
            )
            ...
            if mapping:
                try:
                    service.events().update(
                        calendarId="primary", eventId=mapping.google_event_id, body=body
                    ).execute()
                except Exception as e:
...
def calendar_callback(
    ...
    try:
        ...
        if os.environ.get("GOOGLE_CLIENT_ID") is None:
            ...
            flow.fetch_token(
                authorization_response=auth_response, code_verifier=google_code_verifier
            )
